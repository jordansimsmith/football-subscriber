...
public class FixtureMergerTest
{
    ...
    public FixtureMergerTest()
    {
        ...
        _subject = new FixtureMerger(
            _mockFixtureRepository.Object,
            _mockFixtureChangeNotificationService.Object
        );
    }
    ...
    public async Task MergerAsync_Should_Merge_Fixtures_Correctly()
    {
        ...
        var oldFixtures = new[]
        {
            new Fixture { ApiId = 1, Address = "address 1" },
            new Fixture { ApiId = 3, Address = "address 3" },
            new Fixture { ApiId = 5, Address = "address 5" }
        };

        var newFixtures = new[]
        {
            new Fixture { ApiId = 3, Address = "address 3 updated" },
            new Fixture { ApiId = 4, Address = "address 4" },
            new Fixture { ApiId = 5, Address = "address 5 updated" }
        };
        ...
        _mockFixtureRepository.Verify(
            x => x.AddAsync(It.Is<Fixture>(f => f.ApiId == 4)),
            Times.Once
        );
